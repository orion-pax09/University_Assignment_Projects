...
unsigned char* double_Array(const unsigned char* arr, int size){
    ...
    for (int i = 0; i < newsize; i++){  // FIXED: removed && i < 40
        copy[2*i] = arr[i];
        copy[2*i+1] = arr[i];
    }
    return copy;   
}
//shrinking function skipping every odd  indexed element array
...
{
    // Double sampling rate only - this makes audio play faster/higher pitch
    int new_sampling_rate = 2 * samplingrate;

    string newFile = "upsampled_" + filename;

    writeWavFile(newFile.c_str(), arr, size, new_sampling_rate);
}
...
void downsampleaudio(string& filename, unsigned char* arr, int size, int samplingrate)
{
    ...
    // Let shrinking_Array handle size calculation to avoid mismatch
    unsigned char* downsampling = shrinking_Array(arr, size);
    int new_Size = (size % 2 == 1) ? (size + 1) / 2 : size / 2;  // FIXED: matches shrinking_Array logic exactly
